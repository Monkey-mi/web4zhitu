...
public class BulletinCacheDaoImpl extends BaseCacheDaoImpl<OpMsgBulletin> implements
	...
	public List<OpMsgBulletin> queryTheme(RowSelection rowSelection) {
		return getRedisTemplate().opsForList().range(CacheKeies.OP_MSG_THEME, 
				rowSelection.getFirstRow(),
				rowSelection.getMaxRow() - 1);
	}
	
	@Override
	public List<OpMsgBulletin> queryUserTheme(RowSelection rowSelection) {
		return getRedisTemplate().opsForList().range(CacheKeies.OP_MSG_USER_THEME, 
				rowSelection.getFirstRow(),
				rowSelection.getMaxRow() - 1);
	}
