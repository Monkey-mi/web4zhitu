	...
	$.formValidator = {
		...
		sustainType : function(id, setting) {
			...
			case "CompareValidator":
				return ((srcTag == "INPUT" || srcTag == "TEXTAREA") ? (stype != "checkbox" && stype != "radio")
						: false);
			case "AjaxValidator":
				...
				$("#" + settings.formid).submit(
						function() {
							return $.formValidator
									.pageIsValid(settings.validatorgroup);
						});
			}
			;
		...
		getLength : function(id) {
			...
			case "file":
				var val = srcjo.val();
				var initConfig = $('body')
						.data(elem.settings[0].validatorgroup);
				if (initConfig.wideword) {
					for ( var i = 0; i < val.length; i++) {
						len = len
								+ ((val.charCodeAt(i) >= 0x4e00 && val
										.charCodeAt(i) <= 0x9fa5) ? 2 : 1);
						/*
						 * if (val.charCodeAt(i) >= 0x4e00 && val.charCodeAt(i) <=
						 * 0x9fa5) len = len + 2; else len = len + 1;
						 */
					}
				} else {
					len = val.length;
		...
		isEmpty : function(id) {
			return ($("#" + id).get(0).settings[0].empty && $.formValidator
					.getLength(id) == 0);
		},
									...
									if (name_list.indexOf("^" + name + "^") == -1) {
										...
										if (returnObj) {
											...
											if (!returnObj.isvalid) {
												...
												errorlist[errorlist.length] = returnObj.errormsg == "" ? returnObj.setting.onerror
														: returnObj.errormsg;
											}
											// 为了解决使用同个TIP提示问题:后面的成功或失败都不覆盖前面的失败
											if (!initConfig.alertmessage) {
												var tipid = this.settings[0].tipid;
												if (error_tip.indexOf("^"
														+ tipid + "^") == -1) {
													if (!returnObj.isvalid) {
														error_tip = error_tip
																+ tipid + "^";
		...
		regexValid : function(returnObj) {
			...
			if (elem.settings[0].empty && elem.value == "") {
				...
				$.each(regexpresslist, function(i, val) {
					...
					isvalid = (new RegExp(r, setting.param)).test($("#" + id)
							.val());
				...
				.each(function(e) {
					...
					if (initConfig.autotip) {
						if (!initConfig.tidymode) {
							...
							if ($("body [id=" + tip + "]").length == 0) {
								var afterid = setting_temp.relativeid ? setting_temp.relativeid
										: this.id;
								var offset = $("#" + afterid).position();
								var y = offset.top;
								var x = $("#" + afterid).width() + offset.left;
								$("<div class='formValidateTip'></div>")
										.appendTo($("body"))
										.css({
											left : x + "px",
											top : y + "px"
					...
					if (srcTag == "input" || srcTag == "textarea") {
						...
						jqobj.bind({
							...
							focus : function() {
								if (!initConfig.alertmessage) {
									$.formValidator.setTipState(this,
											"onFocus", setting.onfocus);
								}
								;
							},
							// 失去焦点
